...
function Orb({ icon, accent, reduced }) {
  ...
  return (
    <div style={{ position: 'relative', width: 120, height: 120, margin: '0 auto 36px' }}>
      <div style={{ position: 'absolute', inset: -10, borderRadius: '50%', background: `radial-gradient(circle, ${blue ? 'rgba(71,199,252,0.18)' : 'rgba(238,140,70,0.18)'}, transparent 70%)`, filter: 'blur(4px)' }} />
      <div
...
export default function Onboarding({ onNavigate }) {
  ...
  return (
    <div style={{ background: colors.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(60% 50% at 50% 38%, ${blueTheme ? 'rgba(71,199,252,0.09)' : 'rgba(238,140,70,0.10)'}, transparent)`, transition: 'background 0.6s ease', pointerEvents: 'none' }} />
      <FilmGrain />
